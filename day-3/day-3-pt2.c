...
imatch_t find_contiguous_number(int idx,
                                int w,
                                ivec2_t* pos,
                                char* flattened_content) {
  ...
  imatch_t result = {start + 1, end};
  return result;
}

int matches_eql(imatch_t* a, imatch_t* b) {
  return a->start == b->start && a->end == b->end;
}

int main() {
  char* content = read_file("./day-3-pt2-input.txt");
    ...
    if (c == '*') {
      ...
      for (int j = 0; j < N_DIRECTIONS; j++) {
        ...
        if (pos.x >= 0 && pos.x < w && pos.y >= 0 && pos.y < h) {
          ...
          if (is_number(c2)) {
            imatch_t match =
                find_contiguous_number(idx, w, &pos, flattened_content);
            if (matches_eql(&match, &matches[0]) ||
                matches_eql(&match, &matches[1])) {
              continue;
            } else if (count < 2) {
              char* match_str =
                  substr(match.start, match.end, flattened_content);
              ratio *= strtol(match_str, NULL, 10);
              matches[count] = match;
              count++;
